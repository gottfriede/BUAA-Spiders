...
def login(username, password, url, chrome) :
    ...
    codeSrc = chrome.find_element_by_class_name("code").get_attribute("src")
    if codeSrc!=None :
        response = req.get(codeSrc)
        img = Image.open(BytesIO(response.content))
        ocrResult = tesserocr.image_to_text(img)
        if ocrResult == "" :
            return -1
        code_location.send_keys()
